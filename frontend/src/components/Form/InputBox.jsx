...
export const InputBox = ({ label,type,name,id, placeholder, onChange,className,value, error}) => {
    ...
    {error && <input type={type} name={name} id={id} placeholder={placeholder} onChange={onChange} className={`h-10 border border-rose-500 mt-1 rounded px-4 w-full bg-gray-50 ${className}`} value={value}  />}
  </div>
}